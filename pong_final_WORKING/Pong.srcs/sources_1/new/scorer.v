...
module scorer(clk_100MHz, reset, score_A, score_B, seg, enables);
    input clk_100MHz;
    input reset;
    ...
    reg [3:0] score;
    output reg [3:0] enables; //MENA MODIFIED THIS TO BE OUTPUT
    ...
    // Counters
    always @ (posedge score_A or posedge reset) begin
      if (reset)  begin
        score_A_int <= 0;
      end
      else
    ...
    always @ (posedge score_B or posedge reset) begin
          if (reset)  begin
            score_B_int <= 0;
          end
          else
    ...
    always @ (enables or score_A_int or score_B_int) begin
      case (enables)
